...
namespace oschina2022
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            var pow_sig = 25;
            Console.WriteLine($"项目ID:{objId}{Environment.NewLine}用户ID:{g_user_id}{Environment.NewLine}:登录信息:{oscid}{Environment.NewLine}并行计算{pow_batch}个线程，每线程{pow_sig}。{Environment.NewLine}");
            ...
            do
            {
                ...
                Parallel.For(1, 26,  _ =>
                {
                    ...
                    if (osc != null)
                    {
                        ...
                        _count++;
                        _scores++;
                        Console.WriteLine($"CPU{Environment.ProcessId}线程{Thread.CurrentThread.ManagedThreadId}算得热度值{scores}，耗时{DateTime.Now.Subtract(dt2).TotalSeconds}");
                    }
                });
